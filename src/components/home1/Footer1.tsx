    ...
    <div className="flex gap-3">
      {present.map((k) => (
        <a
          ...
          aria-label={k}
          className="flex h-10 w-10 items-center justify-center rounded-full border border-white/15 text-white/70 transition-colors hover:border-amber-300 hover:bg-amber-300 hover:text-[#0b1f3a]"
        >
...
export default function Footer1({ siteSettings }: { siteSettings: any }) {
  const phone = siteSettings?.phone?.[0]?.number
  const email = siteSettings?.email?.[0]?.address
  const cols = [
    { t: 'Academics', links: [['Programmes', '/programs'], ['Departments', '/departments'], ['Faculty', '/faculty'], ['Research', '/departments']] },
  ...
  return (
    <footer className="bg-[#081627] pt-20 text-white">
        ...
        <div className="grid grid-cols-1 gap-12 border-b border-white/10 pb-16 lg:grid-cols-12">
          ...
          <div className="lg:col-span-4">
            ...
            <p className="mt-5 max-w-sm leading-relaxed text-white/60">
              SITASRM Engineering &amp; Research Institute — shaping engineers, innovators and leaders
          ...
          {cols.map((c) => (
            <div key={c.t} className="lg:col-span-2">
              <p className="font-serif-display text-sm font-semibold uppercase tracking-widest text-amber-300">{c.t}</p>
          ...
          <div className="lg:col-span-2">
            <p className="font-serif-display text-sm font-semibold uppercase tracking-widest text-amber-300">Contact</p>
            ...
            <Link href="/contact" className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-amber-300 hover:text-amber-200">
              Plan a visit <ArrowRight className="h-4 w-4" aria-hidden="true" />
            </Link>
          </div>
        </div>

        <div className="flex flex-col items-center justify-between gap-4 py-7 text-sm text-white/40 sm:flex-row">
